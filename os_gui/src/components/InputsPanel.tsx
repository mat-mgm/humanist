...
import { ArrowDownToLine, File, Folder, FolderTree, Trash2, Upload } from 'lucide-react';
...
function InputDraftCard({
  ...
  const badgeStyle = statusBadgeStyle(draft.stage);
  const hasBlobSource = Boolean(draft.blobAttachment || draft.sourcePath || draft.fileName || draft.bytes);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderBottom: draft.expanded ? '1px solid var(--border)' : 'none',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
          ...
          >
            {draft.kind === 'create' ? 'New entity' : ((draft.fileName ?? draft.label) || 'Imported file')}
          </button>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            onClick={() => removeInputDraft(draft.jobId)}
            title="Remove draft"
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: '12px 12px 10px', display: 'grid', gap: 10 }}>
          <div style={{ fontSize: 12, color: 'var(--text-hint)', lineHeight: 1.5 }}>
            New entities and file imports land in the same draft queue. Imports create entities with attached blob traits.
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            <button
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                onKeyDown={e => {
                  ...
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    // If the dropdown is open and they actively selected something with arrows or simply have 0 selected,
                    // they might expect Enter to just use that. But if they just typed an exact path, 
                    // it shouldn't be overridden if they just want what they typed.
                    // However, we'll keep the current behavior where Enter uses the selected completion.
                    // Actually, if we use what they typed if it differs? No, let's keep it as selectedPath.
                    let selectedPath: string | undefined = pathCompletions[activePathCompletionIdx]?.path;
                    
                    // But if they didn't navigate (index is 0) and what they typed is NOT the prefix
                    // or they just want what they typed, it's safer to use selectedPath if they navigated,
                    // but we don't track if they navigated. Let's just use what's highlighted.
                    // Wait, currently if activePathCompletionIdx is 0, selectedPath is pathCompletions[0].path.
                    if (pathCompletions.length > 0) {
                      // Apply visually but then add
                      applyCompletion(activePathCompletionIdx);
                    } else {
      ...
      {inputDrafts.length === 0 ? (
        <div style={{
          ...
          Add a new entity or select files to start the entity draft queue.
        </div>
      ) : (
        <div style={{ display: 'grid', gap: 16 }}>
          <div style={{ fontSize: 12, color: 'var(--text-hint)', padding: '0 2px' }}>
            {readyCount} editable cards
          </div>
